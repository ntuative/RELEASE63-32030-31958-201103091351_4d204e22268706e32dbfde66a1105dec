package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      protected var var_1510:IMouseCursor;
      
      protected var var_811:IWindowContextStateListener;
      ...
      public function MouseEventProcessor(param1:IMouseCursor)
      {
         ...
         {
            var_324 = new Array();
            var_324[0] = MouseCursorType.const_301;
            var_324[1] = MouseCursorType.const_31;
            var_324[2] = MouseCursorType.const_301;
            var_324[3] = MouseCursorType.const_301;
            var_324[4] = MouseCursorType.const_301;
            var_324[5] = MouseCursorType.const_31;
            var_324[6] = MouseCursorType.const_301;
         }
         if(var_280 == null)
         {
            var_280 = new Array();
            var_280[0] = WindowState.const_110;
            var_280[1] = WindowState.const_83;
            var_280[2] = WindowState.const_96;
            var_280[3] = WindowState.const_71;
            var_280[4] = WindowState.const_52;
            var_280[5] = WindowState.const_98;
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_168 = state.var_1235 as WindowController;
         this.var_158 = state.renderer;
         this.var_811 = state.var_1234;
         eventQueue.begin();
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_657.x || event.stageY != this.var_657.y)
            {
               ...
               array = new Array();
               this.var_137.groupParameterFilteredChildrenUnderPoint(this.var_657,array,WindowParam.const_30);
            }
            index = array != null ? int(array.length) : 0;
            this.var_1510.type = MouseCursorType.const_31;
            if(index == 0)
            {
               if(event.type == MouseEvent.MOUSE_MOVE)
               {
                  if(this.var_74 != this.var_137 && !this.var_74.disposed)
                  {
                     ...
                     this.var_74 = WindowController(this.var_137);
                  }
               }
            }
            while(--index > -1)
               ...
               if(child != null && child.visible)
               {
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK)
                  {
                     if(this.var_74 && !this.var_74.disposed)
                     {
                        ...
                        {
                           this.var_811.mouseEventReceived(event.type,this.var_74);
                        }
                     }
                  }
                  temp = child.parent;
                  ...
                  if(this.var_74 is IInteractiveWindow)
                  {
                     try
                     {
                        this.var_1510.type = getMouseCursorByState(this.var_74.state);
                     }
                     catch(e:Error)
                     {
                        var_1510.type = MouseCursorType.const_31;
                     }
                  }
         ...
         state.var_1235 = this.var_168;
         state.renderer = this.var_158;
         state.var_1234 = this.var_811;
      }
      ...
      private function passMouseEvent(param1:WindowController, param2:MouseEvent, param3:Boolean = false) : WindowController
      {
         ...
         if(param1.testParamFlag(WindowParam.const_550))
         {
