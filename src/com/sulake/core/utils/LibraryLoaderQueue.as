package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1580:int = 4;
      ...
      {
         return this.var_429.length + this.var_277.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_277.length < const_1580 && this.var_429.length > 0)
            {
               _loc1_ = this.var_429.shift();
               this.var_277.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_429.indexOf(loader);
            if(index > -1)
            {
               this.var_429.splice(index,1);
            }
            index = this.var_277.indexOf(loader);
            if(index > -1)
            {
               this.var_277.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_429)
            {
               if(param2)
               {
