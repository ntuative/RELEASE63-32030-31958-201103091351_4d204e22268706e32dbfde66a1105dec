package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private static var var_1482:String = "%";
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. URL " + this.getKey("external.variables.txt") + "HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1483);
      }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         for each(line in rows)
         {
            if(line.substr(0,1) != "#")
            {
               ...
               if(pair[0].length > 0 && pair[1].length > 0)
               {
                  ...
                  if(key == "configuration.readonly" && value == "HabboHelpCom_help_faq_entry_xml")
                  {
                     readOnly = true;
                  }
         ...
         if(this.getKey("local.environment") == "HabboHelpCom_help_faq_entry_xml")
         {
      ...
      {
         var _loc2_:String = this.var_1202[param1] as String;
         return _loc2_ != null;
      }
      
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         var _loc4_:String = this.var_1202[param1] as String;
         if(_loc4_ == null)
         {
            _loc4_ = param2;
         }
      ...
      public function setKey(param1:String, param2:String, param3:Boolean = false, param4:Boolean = false) : void
      {
         ...
         if(this.var_1906.indexOf(param1) < 0 || param3)
         {
            this.var_1202[param1] = param2;
         }
         if(param3)
         {
            this.var_1906.push(param1);
         }
      }
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1482);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1482,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
