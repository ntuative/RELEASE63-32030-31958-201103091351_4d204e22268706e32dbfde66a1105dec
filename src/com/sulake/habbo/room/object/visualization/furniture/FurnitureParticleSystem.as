package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_247:IRoomObjectSprite;
      ...
      private var var_1082:Number = 1.0;
      
      private var var_579:BitmapData;
      
      private var var_703:ColorTransform;
      
      private var var_1333:ColorTransform;
      
      private var var_1648:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_674:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_703 = new ColorTransform();
         this.var_703.alphaMultiplier = 1;
         this.var_1333 = new ColorTransform();
         this.var_1648 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1083)
         {
            _loc1_.dispose();
         }
         this.var_1083 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_96 = this.var_1083[param1];
         this.var_878 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1649 >= 0)
         {
            this.var_247 = this._visualization.getSprite(this.var_1649);
            if(this.var_247 && this.var_247.asset)
            {
               if(this.var_247.width <= 1 || this.var_247.height <= 1)
               {
                  return;
               }
               if(this.var_87 == null)
               {
                  this.var_87 = this.var_247.asset.clone();
                  if(this.var_703.alphaMultiplier != 1)
                  {
                     this.var_579 = new BitmapData(this.var_87.width,this.var_87.height,true,4278190080);
                  }
               }
               this.var_2148 = -this.var_247.offsetX;
               this.var_2149 = -this.var_247.offsetY;
               this.var_247.asset = this.var_87;
            }
            if(this.var_87)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_96.y * this.var_1082;
         }
         return 0;
      }
      ...
      {
         if(!this.var_96 || !this.var_247)
         {
            return;
         }
         if(this.var_87 && this.var_247.asset != this.var_87)
         {
            this.var_247.asset = this.var_87;
         }
         if(this.var_878)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_878 && this.var_96.hasIgnited)
         {
            this.var_878 = true;
         }
         _loc8_ = this.var_1130 * this.var_1082;
         this.var_96.update();
         if(this.var_878)
         {
            if(this.var_96.roomObjectSpriteId >= 0)
            ...
            {
               this.var_87.draw(this.var_579,this.var_1648,this.var_703,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_96.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2148 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1082;
               _loc4_ = this.var_2149 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1082;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1333.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_87.draw(_loc7_,this._translationMatrix,this.var_1333,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2294 = parseInt(param1.@size);
         this.var_1649 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1130 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1082 = this.var_2294 / 64;
         this.var_674 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_674 = Math.min(this.var_674,1);
         this.var_703.alphaMultiplier = this.var_674;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "HabboHelpCom_help_faq_entry_xml" ? true : false;
               _loc21_ = [];
